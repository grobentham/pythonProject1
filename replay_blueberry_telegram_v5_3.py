...
def find_v52_source() -> Path:
    ...
    candidates = [
        ...
        if p.is_file() and "v5_3" not in p.name.lower() and "v53" not in p.name.lower()
    ]
    if not candidates:
...
def write_v53_extras(engine):
    ...
    if not result_dirs:
        print("[V5.3] V5.2 reporter result directory was not found; execution completed but extras could not be attached.")
        return None
    ...
    (out / "V53_SUCCESS.txt").write_text(
        "V5.3 execution-integrity overlay completed.\n", encoding="utf-8"
    )
...
def main():
    ...
    print(f"[V5.3] Frozen compiler source: {v52_source}", flush=True)
    ...
    class CapturingV53Engine(V53PortfolioEngine):
        def __init__(self, *args, **kwargs):
            global LAST_ENGINE
            super().__init__(*args, **kwargs)
            LAST_ENGINE = self

    # Freeze the V4.1/V5.2 language compiler and replace ONLY its execution engine.
